...
def compute_invalid_moves(state, player, ko_protect=None):
    ...
    # Get all groups
    all_own_groups, num_own_groups = label(state[player])
    all_opp_groups, num_opp_groups = label(state[1 - player])
...
def batch_compute_invalid_moves(batch_state, batch_player, batch_ko_protect):
    ...
    # Get all groups
    batch_all_own_groups, _ = label(batch_state[batch_idcs, batch_player], group_struct)
    batch_all_opp_groups, _ = label(batch_state[batch_idcs, 1 - batch_player], group_struct)
    ...
    for i, (all_own_groups, all_opp_groups, empties) in batch_data:
        ...
        # Expand the groups such that each group is in its own channel
        for j, l in enumerate(own_labels):
            expanded_own_groups[j] = all_own_groups == l

        for j, l in enumerate(opp_labels):
            expanded_opp_groups[j] = all_opp_groups == l
...
def update_pieces(state, adj_locs, player):
    ...
    all_opp_groups, _ = label(state[opponent])
...
def batch_update_pieces(batch_non_pass, batch_state, batch_adj_locs, batch_player):
    ...
    batch_all_opp_groups, _ = label(batch_state[batch_non_pass, batch_opponent],
                                                         group_struct)
