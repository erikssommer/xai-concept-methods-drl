...
from mcts import MonteCarlo as MCTS
...
def rl_uct():
    ...
    for _ in tqdm(range(start_episode, episodes)):
        ...
        tree = MCTS(root, move_cap, board_size, c, komi, policy_nn)
        ...
        while not game_over:
            ...
            assert curr_state.all() == tree.root_node.state.all()

            for _ in range(simulations):
                tree.simulate()

            best_action_node = tree.make_choice(current_ply)
            distribution = tree.distribution()
            
            # Get the best action node
            ...
            tree.move_root(best_action_node)
